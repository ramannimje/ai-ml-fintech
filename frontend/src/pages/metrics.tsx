...
export function MetricsPage() {
  ...
  const queries = useQueries({ queries: commodities.map((c) => ({ queryKey: ['metrics', c, region], queryFn: () => client.metrics(c, region), staleTime: 300_000 })) });
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold">Model Metrics ({region.toUpperCase()})</h1>
      <div className="overflow-hidden rounded-xl border border-slate-800">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-900 text-slate-300"><tr><th className="p-3">Model</th><th>RMSE</th><th>MAE</th><th>Last trained</th><th>Status</th></tr></thead>
          <tbody>
            {queries.map((q, i) => (
