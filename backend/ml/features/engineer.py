...
def add_macro_features(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    # placeholder exogenous proxies; pipeline-ready columns
    out["inflation_index"] = np.linspace(100, 112, len(out))
    out["usd_index"] = np.linspace(102, 106, len(out))
    out["interest_rate"] = np.linspace(2.1, 4.5, len(out))
    return out
